...
#[tauri::command]
pub async fn delete_notification(state: State<'_, AppState>, id: String) -> AppResult<()> {
    notifications::delete_notification(&state.db, &id).await
}

#[tauri::command]
pub async fn delete_all_read_notifications(state: State<'_, AppState>) -> AppResult<u64> {
