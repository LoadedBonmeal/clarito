/**
 * Companie nouă — design-system form page (no dedicated prototype; follows the
 * Contacts modal .fgrid/.field + ANAF-autofill conventions and the InvoiceNew
 * full-page-form layout):
 *   .page-head (.crumb "Companii › Companie nouă" + h1 + sub + .head-actions
 *   Renunță / btn-dark "Salvează compania") → .scr-card "Identificare"
 *   (CUI cu autofill ANAF .in-wrap + buton ANAF ↓, Reg. Com., denumiri,
 *   plătitor TVA) → "Adresă" → "Contact & bancă" → "Facturare" (serie facturi,
 *   regim fiscal micro/profit) → .banner notă SPV/logo după salvare.
 *
 * ALL wiring preserved: react-hook-form + Zod (CUI regex, IBAN mod-97),
 * CUI lookup → api.companies.fetchAnafData (auto-fill legalName/address/
 * city/county/registryNumber), api.companies.create → invalidate + navigate
 * la detaliu. SPV/logo se configurează după salvare (nota informativă —
...
export function CompanyNewPage() {
  ...
  return (
    <div className="main-inner">
      ...
      {/* SPV + logo — se configurează după salvare (fostele etape 2–3 ale wizardului) */}
